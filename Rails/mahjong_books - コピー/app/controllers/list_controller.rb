class ListController < ApplicationController
  ...
  def renmei
    @books = Book.joins(:authors).joins({:authors => :organizations}).where(:organizations => {name: "日本プロ麻雀連盟"}).order("authors.name")
  end

  def others
    @books = Book.joins(:authors).joins({:authors => :organizations}).where(:organizations => {name: nil}).order("authors.name")
  end
